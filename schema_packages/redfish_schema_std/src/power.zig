//! Namespace `Power`.
//!
//! Generated by `redfish-codegen`. DO NOT EDIT.
//!
//! Profile: `redfish_schema_std`.

const std = @import("std");

const core = @import("redfish_core");
const assembly = @import("assembly.zig");
const physical_context = @import("physical_context.zig");
const redundancy = @import("redundancy.zig");
const resource = @import("resource.zig");

pub const InputType = enum {
    /// Alternating Current (AC) input range.
    AC,
    /// Direct Current (DC) input range.
    DC,

    /// A value this package's schema version does not name.
    UnsupportedValue,

    const open = core.OpenEnum(@This());
    pub const jsonParse = open.jsonParse;
    pub const jsonParseFromValue = open.jsonParseFromValue;
};

pub const LineInputVoltageType = enum {
    /// The power supply line input voltage type cannot be determined.
    Unknown,
    /// 100-127V AC input.
    ///
    /// Deprecated in v1_1_0.
    /// This value has been deprecated in favor of AC120V.
    ACLowLine,
    /// 200-240V AC input.
    ///
    /// Deprecated in v1_1_0.
    /// This value has been deprecated in favor of AC240V.
    ACMidLine,
    /// 277V AC input.
    ///
    /// Deprecated in v1_1_0.
    /// This value has been deprecated in favor of AC277V.
    ACHighLine,
    /// -48V DC input.
    DCNeg48V,
    /// High-voltage DC input (380V).
    DC380V,
    /// AC 120V nominal input.
    AC120V,
    /// AC 240V nominal input.
    AC240V,
    /// AC 277V nominal input.
    AC277V,
    /// Wide range AC or DC input.
    ACandDCWideRange,
    /// Wide range AC input.
    ACWideRange,
    /// DC 240V nominal input.
    DC240V,

    /// A value this package's schema version does not name.
    UnsupportedValue,

    const open = core.OpenEnum(@This());
    pub const jsonParse = open.jsonParse;
    pub const jsonParseFromValue = open.jsonParseFromValue;
};

pub const PowerLimitException = enum {
    /// Take no action when the limit is exceeded.
    NoAction,
    /// Turn the power off immediately when the limit is exceeded.
    HardPowerOff,
    /// Log an event when the limit is exceeded, but take no further action.
    LogEventOnly,
    /// Take an OEM-defined action.
    Oem,

    /// A value this package's schema version does not name.
    UnsupportedValue,

    const open = core.OpenEnum(@This());
    pub const jsonParse = open.jsonParse;
    pub const jsonParseFromValue = open.jsonParseFromValue;
};

pub const PowerSupplyType = enum {
    /// The power supply type cannot be determined.
    Unknown,
    /// Alternating Current (AC) power supply.
    AC,
    /// Direct Current (DC) power supply.
    DC,
    /// The power supply supports both DC and AC.
    ACorDC,

    /// A value this package's schema version does not name.
    UnsupportedValue,

    const open = core.OpenEnum(@This());
    pub const jsonParse = open.jsonParse;
    pub const jsonParseFromValue = open.jsonParseFromValue;
};

/// Arguments to `Power.PowerSupplyReset`.
///
/// This action resets the targeted power supply.
pub const PowerPowerSupplyResetAction = struct {
    /// The type of reset.
    ///
    /// This parameter shall contain the type of reset.  The service can accept a request without the parameter and shall perform a `GracefulRestart`.
    ResetType: ?resource.ResetType = null,
    /// The `MemberId` of the power supply within the `PowerSupplies` array on which to perform the reset.
    ///
    /// This parameter shall contain the identifier of the member within the `PowerSupplies` array on which to perform the reset.
    MemberId: ?[]const u8 = null,

    pub const jsonStringify = core.Payload(@This()).jsonStringify;
};

/// The available actions for this resource.
///
/// This type shall contain the available actions for this resource.
pub const Actions = struct {
    /// The available OEM-specific actions for this resource.
    ///
    /// This property shall contain the available OEM-specific actions for this resource.
    Oem: ?OemActions = null,
    /// This action resets the targeted power supply.
    ///
    /// This action shall reset a power supply specified by the `MemberId` from the `PowerSupplies` array.  A `GracefulRestart` `ResetType` shall reset the power supply but shall not affect the power output.  A `ForceRestart` `ResetType` can affect the power supply output.
    @"#Power.PowerSupplyReset": ?core.Action(PowerPowerSupplyResetAction, std.json.Value) = null,

    /// This action resets the targeted power supply.
    ///
    /// This action shall reset a power supply specified by the `MemberId` from the `PowerSupplies` array.  A `GracefulRestart` `ResetType` shall reset the power supply but shall not affect the power output.  A `ForceRestart` `ResetType` can affect the power supply output.
    ///
    /// Returns `error.ActionNotSupported` if the service did not
    /// advertise this action on the resource.
    pub fn powerSupplyReset(
        self: @This(),
        gpa: std.mem.Allocator,
        transport: *core.BmcTransport,
        params: PowerPowerSupplyResetAction,
    ) !core.Owned(core.ModificationResponse(std.json.Value)) {
        const target = self.@"#Power.PowerSupplyReset" orelse return error.ActionNotSupported;
        return core.bmc.invokeAction(gpa, transport, target, params);
    }
};

/// This type describes an input range for a power supply.
///
/// This type shall describe an input range that the associated power supply can utilize.
pub const InputRange = struct {
    /// The Input type (AC or DC).
    ///
    /// This property shall contain the input type (AC or DC) of the associated range.
    InputType: ?InputType = null,
    /// The minimum line input voltage at which this power supply input range is effective.
    ///
    /// This property shall contain the value, in volt units, of the minimum line input voltage that the power supply is capable of consuming for this range.
    MinimumVoltage: ?core.Decimal = null,
    /// The maximum line input voltage at which this power supply input range is effective.
    ///
    /// This property shall contain the value, in volt units, of the maximum line input voltage that the power supply is capable of consuming for this range.
    MaximumVoltage: ?core.Decimal = null,
    /// The minimum line input frequency at which this power supply input range is effective.
    ///
    /// This property shall contain the value, in hertz units, of the minimum line input frequency that the power supply is capable of consuming for this range.
    MinimumFrequencyHz: ?core.Decimal = null,
    /// The maximum line input frequency at which this power supply input range is effective.
    ///
    /// This property shall contain the value, in hertz units, of the maximum line input frequency that the power supply is capable of consuming for this range.
    MaximumFrequencyHz: ?core.Decimal = null,
    /// The maximum capacity of this power supply when operating in this input range.
    ///
    /// This property shall contain the maximum amount of power, in watt units, that the associated power supply is rated to deliver while operating in this input range.
    OutputWattage: ?core.Decimal = null,
    /// The OEM extension property.
    ///
    /// This property shall contain the OEM extensions.  All values for properties contained in this object shall conform to the Redfish Specification-described requirements.
    Oem: ?resource.Oem = null,
};

/// What a client may change of `Power.InputRange`.
///
/// This type describes an input range for a power supply.
pub const InputRangeUpdate = struct {
    /// The OEM extension property.
    ///
    /// This property shall contain the OEM extensions.  All values for properties contained in this object shall conform to the Redfish Specification-described requirements.
    Oem: ?resource.OemUpdate = null,

    pub const jsonStringify = core.Payload(@This()).jsonStringify;
};

/// The available OEM-specific actions for this resource.
///
/// This type shall contain the available OEM-specific actions for this resource.
pub const OemActions = struct {
    /// Whatever the service sent that this schema version does not name.
    additional_properties: core.AdditionalProperties = .{},

    const open = core.OpenStruct(@This());
    pub const jsonParse = open.jsonParse;
    pub const jsonParseFromValue = open.jsonParseFromValue;
    pub const jsonStringify = open.jsonStringify;
};

/// The available actions for this resource.
///
/// This type shall contain the available actions for this resource.
pub const PowerControlActions = struct {
    /// The available OEM-specific actions for this resource.
    ///
    /// This property shall contain the available OEM-specific actions for this resource.
    Oem: ?PowerControlOemActions = null,
};

/// What a client may change of `Power.PowerControlActions`.
///
/// The available actions for this resource.
pub const PowerControlActionsUpdate = struct {
    /// The available OEM-specific actions for this resource.
    ///
    /// This property shall contain the available OEM-specific actions for this resource.
    Oem: ?PowerControlOemActionsUpdate = null,

    pub const jsonStringify = core.Payload(@This()).jsonStringify;
};

/// The available OEM-specific actions for this resource.
///
/// This type shall contain the available OEM-specific actions for this resource.
pub const PowerControlOemActions = struct {
    /// Whatever the service sent that this schema version does not name.
    additional_properties: core.AdditionalProperties = .{},

    const open = core.OpenStruct(@This());
    pub const jsonParse = open.jsonParse;
    pub const jsonParseFromValue = open.jsonParseFromValue;
    pub const jsonStringify = open.jsonStringify;
};

/// What a client may change of `Power.PowerControlOemActions`.
///
/// The available OEM-specific actions for this resource.
pub const PowerControlOemActionsUpdate = struct {
    /// Members to send that this schema version does not name.
    additional_properties: core.AdditionalProperties = .{},

    pub const jsonStringify = core.Payload(@This()).jsonStringify;
};

/// The power limit status and configuration information for the chassis.
///
/// This type shall contain power limit status and configuration information for this chassis.
pub const PowerLimit = struct {
    /// The power limit, in watt units.  If `null`, power capping is disabled.
    ///
    /// This property shall represent the power capping limit, in watt units, for the resource.  If `null`, power capping shall be disabled.
    LimitInWatts: ?core.Decimal = null,
    /// The action that is taken if the power cannot be maintained below the `LimitInWatts`.
    ///
    /// This property shall represent the action to be taken if the resource power consumption cannot be limited below the specified limit after several correction time periods.
    LimitException: ?PowerLimitException = null,
    /// The time required for the limiting process to reduce power consumption to below the limit.
    ///
    /// This property shall represent the time interval in ms required for the limiting process to react and reduce the power consumption below the limit.
    CorrectionInMs: ?i64 = null,
};

/// What a client may change of `Power.PowerLimit`.
///
/// The power limit status and configuration information for the chassis.
pub const PowerLimitUpdate = struct {
    /// The power limit, in watt units.  If `null`, power capping is disabled.
    ///
    /// This property shall represent the power capping limit, in watt units, for the resource.  If `null`, power capping shall be disabled.
    LimitInWatts: core.Nullable(core.Decimal) = .absent,
    /// The action that is taken if the power cannot be maintained below the `LimitInWatts`.
    ///
    /// This property shall represent the action to be taken if the resource power consumption cannot be limited below the specified limit after several correction time periods.
    LimitException: core.Nullable(PowerLimitException) = .absent,
    /// The time required for the limiting process to reduce power consumption to below the limit.
    ///
    /// This property shall represent the time interval in ms required for the limiting process to react and reduce the power consumption below the limit.
    CorrectionInMs: core.Nullable(i64) = .absent,

    pub const jsonStringify = core.Payload(@This()).jsonStringify;
};

/// The power metrics for a resource.
///
/// This type shall contain power metrics for power readings, such as interval, minimum, maximum, and average power consumption, for a resource.
pub const PowerMetric = struct {
    /// The time interval, or window, over which the power metrics are measured.
    ///
    /// This property shall represent the time interval or window, in minutes, over which the power metrics are measured.
    IntervalInMin: ?i64 = null,
    /// The lowest power consumption level, in watt units, over the measurement window that occurred within the last `IntervalInMin` minutes.
    ///
    /// This property shall represent the minimum power level, in watt units, that occurred within the last `IntervalInMin` minutes.
    MinConsumedWatts: ?core.Decimal = null,
    /// The highest power consumption level, in watt units, that has occurred over the measurement window within the last `IntervalInMin` minutes.
    ///
    /// This property shall represent the maximum power level, in watt units, that occurred within the last `IntervalInMin` minutes.
    MaxConsumedWatts: ?core.Decimal = null,
    /// The average power level over the measurement window over the last `IntervalInMin` minutes.
    ///
    /// This property shall represent the average power level that occurred over the last `IntervalInMin` minutes.
    AverageConsumedWatts: ?core.Decimal = null,
};

/// The available actions for this resource.
///
/// This type shall contain the available actions for this resource.
pub const PowerSupplyActions = struct {
    /// The available OEM-specific actions for this resource.
    ///
    /// This property shall contain the available OEM-specific actions for this resource.
    Oem: ?PowerSupplyOemActions = null,
};

/// What a client may change of `Power.PowerSupplyActions`.
///
/// The available actions for this resource.
pub const PowerSupplyActionsUpdate = struct {
    /// The available OEM-specific actions for this resource.
    ///
    /// This property shall contain the available OEM-specific actions for this resource.
    Oem: ?PowerSupplyOemActionsUpdate = null,

    pub const jsonStringify = core.Payload(@This()).jsonStringify;
};

/// The available OEM-specific actions for this resource.
///
/// This type shall contain the available OEM-specific actions for this resource.
pub const PowerSupplyOemActions = struct {
    /// Whatever the service sent that this schema version does not name.
    additional_properties: core.AdditionalProperties = .{},

    const open = core.OpenStruct(@This());
    pub const jsonParse = open.jsonParse;
    pub const jsonParseFromValue = open.jsonParseFromValue;
    pub const jsonStringify = open.jsonStringify;
};

/// What a client may change of `Power.PowerSupplyOemActions`.
///
/// The available OEM-specific actions for this resource.
pub const PowerSupplyOemActionsUpdate = struct {
    /// Members to send that this schema version does not name.
    additional_properties: core.AdditionalProperties = .{},

    pub const jsonStringify = core.Payload(@This()).jsonStringify;
};

/// The available actions for this resource.
///
/// This type shall contain the available actions for this resource.
pub const VoltageActions = struct {
    /// The available OEM-specific actions for this resource.
    ///
    /// This property shall contain the available OEM-specific actions for this resource.
    Oem: ?VoltageOemActions = null,
};

/// What a client may change of `Power.VoltageActions`.
///
/// The available actions for this resource.
pub const VoltageActionsUpdate = struct {
    /// The available OEM-specific actions for this resource.
    ///
    /// This property shall contain the available OEM-specific actions for this resource.
    Oem: ?VoltageOemActionsUpdate = null,

    pub const jsonStringify = core.Payload(@This()).jsonStringify;
};

/// The available OEM-specific actions for this resource.
///
/// This type shall contain the available OEM-specific actions for this resource.
pub const VoltageOemActions = struct {
    /// Whatever the service sent that this schema version does not name.
    additional_properties: core.AdditionalProperties = .{},

    const open = core.OpenStruct(@This());
    pub const jsonParse = open.jsonParse;
    pub const jsonParseFromValue = open.jsonParseFromValue;
    pub const jsonStringify = open.jsonStringify;
};

/// What a client may change of `Power.VoltageOemActions`.
///
/// The available OEM-specific actions for this resource.
pub const VoltageOemActionsUpdate = struct {
    /// Members to send that this schema version does not name.
    additional_properties: core.AdditionalProperties = .{},

    pub const jsonStringify = core.Payload(@This()).jsonStringify;
};

/// The `Power` schema describes power metrics and represents the properties for power consumption and power limiting.
///
/// This resource shall contain the power metrics for a Redfish implementation.
///
/// Deprecated in v1_7_0.
/// This schema has been deprecated in favor of the `PowerSubsystem` schema.
pub const Power = struct {
    /// Where the resource lives.
    @"@odata.id": ?core.ODataId = null,
    /// The version of the resource this value was read at.
    @"@odata.etag": ?core.ODataETag = null,
    /// The schema version the service implements.
    @"@odata.type": ?[]const u8 = null,
    /// The OEM extension property.
    ///
    /// This property shall contain the OEM extensions.  All values for properties that this object contains shall conform to the Redfish Specification-described requirements.
    Oem: ?resource.Oem = null,
    Id: ?resource.Id = null,
    Description: ?resource.Description = null,
    Name: ?resource.Name = null,
    /// The available actions for this resource.
    ///
    /// This property shall contain the available actions for this resource.
    Actions: ?Actions = null,
    /// The set of power control functions, including power reading and limiting.
    ///
    /// This property shall contain the set of power control readings and settings.
    PowerControl: ?[]const core.NavProperty(PowerControl) = null,
    /// How many members `PowerControl` has in total, which is not how many this response carries.
    @"PowerControl@odata.count": ?i64 = null,
    /// The next page of `PowerControl`. Present only when this response left members out.
    @"PowerControl@odata.nextLink": ?core.ODataId = null,
    /// The set of voltage sensors for this chassis.
    ///
    /// This property shall contain the set of voltage sensors for this chassis.
    Voltages: ?[]const core.NavProperty(Voltage) = null,
    /// How many members `Voltages` has in total, which is not how many this response carries.
    @"Voltages@odata.count": ?i64 = null,
    /// The next page of `Voltages`. Present only when this response left members out.
    @"Voltages@odata.nextLink": ?core.ODataId = null,
    /// The set of power supplies associated with this system or device.
    ///
    /// This property shall contain the set of power supplies associated with this system or device.
    PowerSupplies: ?[]const core.NavProperty(PowerSupply) = null,
    /// How many members `PowerSupplies` has in total, which is not how many this response carries.
    @"PowerSupplies@odata.count": ?i64 = null,
    /// The next page of `PowerSupplies`. Present only when this response left members out.
    @"PowerSupplies@odata.nextLink": ?core.ODataId = null,
    /// The redundancy information for the set of power supplies in this chassis.
    ///
    /// This property shall contain redundancy information for the set of power supplies in this system or device.
    Redundancy: ?[]const core.NavProperty(redundancy.Redundancy) = null,
    /// How many members `Redundancy` has in total, which is not how many this response carries.
    @"Redundancy@odata.count": ?i64 = null,
    /// The next page of `Redundancy`. Present only when this response left members out.
    @"Redundancy@odata.nextLink": ?core.ODataId = null,
};

/// What a client may change of `Power.Power`.
///
/// The `Power` schema describes power metrics and represents the properties for power consumption and power limiting.
///
/// Deprecated in v1_7_0.
/// This schema has been deprecated in favor of the `PowerSubsystem` schema.
pub const PowerUpdate = struct {
    /// The OEM extension property.
    ///
    /// This property shall contain the OEM extensions.  All values for properties that this object contains shall conform to the Redfish Specification-described requirements.
    Oem: ?resource.OemUpdate = null,

    pub const jsonStringify = core.Payload(@This()).jsonStringify;
};

pub const PowerControl = struct {
    /// Where the resource lives.
    @"@odata.id": ?core.ODataId = null,
    /// The version of the resource this value was read at.
    @"@odata.etag": ?core.ODataETag = null,
    /// The OEM extension property.
    ///
    /// This property shall contain the OEM extensions.  All values for properties that this object contains shall conform to the Redfish Specification-described requirements.
    Oem: ?resource.Oem = null,
    /// The unique identifier for the member within an array.
    ///
    /// This property shall contain the unique identifier for this member within an array.  For services supporting Redfish v1.6 or higher, this value shall contain the zero-based array index.
    MemberId: ?[]const u8 = null,
    /// The power control function name.
    ///
    /// This property shall contain the name of the power control function name.
    Name: ?[]const u8 = null,
    /// The actual power that the chassis consumes, in watt units.
    ///
    /// This property shall represent the actual power that the chassis consumes, in watt units.
    PowerConsumedWatts: ?core.Decimal = null,
    /// The potential power, in watt units, that the chassis requests, which might be higher than the current level being consumed because the requested power includes a budget that the chassis wants for future use.
    ///
    /// This property shall represent the amount of power, in watt units, that the chassis currently requests to be budgeted for future use.
    PowerRequestedWatts: ?core.Decimal = null,
    /// The amount of reserve power capacity, in watt units, that remains.  This value is the PowerCapacityWatts value minus the `PowerAllocatedWatts` value.
    ///
    /// This property shall represent the amount of reserve power capacity, in watt units, that remains.  This value is the PowerCapacityWatts value minus the `PowerAllocatedWatts` value.
    PowerAvailableWatts: ?core.Decimal = null,
    /// The total amount of power that can be allocated to the chassis.  This value can be either the power supply capacity or the power budget that an upstream chassis assigns to this chassis.
    ///
    /// This property shall represent the total power capacity that can be allocated to the chassis.
    PowerCapacityWatts: ?core.Decimal = null,
    /// The total amount of power that has been allocated or budgeted to chassis.
    ///
    /// This property shall represent the total power currently allocated or budgeted to the chassis.
    PowerAllocatedWatts: ?core.Decimal = null,
    /// The power readings for this chassis.
    ///
    /// This property shall contain power metrics for power readings, such as interval, minimum, maximum, and average power consumption, for the chassis.
    PowerMetrics: ?PowerMetric = null,
    /// The power limit status and configuration information for this chassis.
    ///
    /// This property shall contain power limit status and configuration information for this chassis.
    PowerLimit: ?PowerLimit = null,
    /// The status and health of the resource and its subordinate or dependent resources.
    ///
    /// This property shall contain any status or health properties of the resource.
    Status: ?resource.Status = null,
    /// The available actions for this resource.
    ///
    /// This property shall contain the available actions for this resource.
    Actions: ?PowerControlActions = null,
    /// The area, device, or set of devices to which this power control applies.
    ///
    /// This property shall contain a description of the affected device(s) or region within the chassis to which this power control applies.
    PhysicalContext: ?physical_context.PhysicalContext = null,
    /// An array of links to resources or objects associated with this power limit.
    ///
    /// This property shall contain an array of links to resources or objects associated with this power limit.
    RelatedItem: ?[]const core.NavProperty(resource.Item) = null,
    /// How many members `RelatedItem` has in total, which is not how many this response carries.
    @"RelatedItem@odata.count": ?i64 = null,
    /// The next page of `RelatedItem`. Present only when this response left members out.
    @"RelatedItem@odata.nextLink": ?core.ODataId = null,
};

/// Details of a power supplies associated with this system or device.
pub const PowerSupply = struct {
    /// Where the resource lives.
    @"@odata.id": ?core.ODataId = null,
    /// The version of the resource this value was read at.
    @"@odata.etag": ?core.ODataETag = null,
    /// The OEM extension property.
    ///
    /// This property shall contain the OEM extensions.  All values for properties that this object contains shall conform to the Redfish Specification-described requirements.
    Oem: ?resource.Oem = null,
    /// The unique identifier for the member within an array.
    ///
    /// This property shall contain the unique identifier for this member within an array.  For services supporting Redfish v1.6 or higher, this value shall contain the zero-based array index.
    MemberId: ?[]const u8 = null,
    /// The name of the power supply.
    ///
    /// This property shall contain a descriptive name for the associated power supply.
    Name: ?[]const u8 = null,
    /// The power supply type (AC or DC).
    ///
    /// This property shall contain the input power type (AC or DC) of the associated power supply.
    PowerSupplyType: ?PowerSupplyType = null,
    /// The line voltage type supported as an input to this power supply.
    ///
    /// This property shall contain the type of input line voltage supported by the associated power supply.
    LineInputVoltageType: ?LineInputVoltageType = null,
    /// The line input voltage at which the power supply is operating.
    ///
    /// This property shall contain the value in volt units of the line input voltage (measured or configured for) that the power supply has been configured to operate with or is currently receiving.
    LineInputVoltage: ?core.Decimal = null,
    /// The maximum capacity of this power supply.
    ///
    /// This property shall contain the maximum amount of power, in watt units, that the associated power supply is rated to deliver.
    PowerCapacityWatts: ?core.Decimal = null,
    /// The average power output of this power supply.
    ///
    /// This property shall contain the average power output, measured in watt units, of the associated power supply.
    LastPowerOutputWatts: ?core.Decimal = null,
    /// The model number for this power supply.
    ///
    /// This property shall contain the model information as defined by the manufacturer for the associated power supply.
    Model: ?[]const u8 = null,
    /// The firmware version for this power supply.
    ///
    /// This property shall contain the firmware version as defined by the manufacturer for the associated power supply.
    FirmwareVersion: ?[]const u8 = null,
    /// The serial number for this power supply.
    ///
    /// This property shall contain the serial number as defined by the manufacturer for the associated power supply.
    SerialNumber: ?[]const u8 = null,
    /// The part number for this power supply.
    ///
    /// This property shall contain the part number as defined by the manufacturer for the associated power supply.
    PartNumber: ?[]const u8 = null,
    /// The spare part number for this power supply.
    ///
    /// This property shall contain the spare or replacement part number as defined by the manufacturer for the associated power supply.
    SparePartNumber: ?[]const u8 = null,
    /// The status and health of the resource and its subordinate or dependent resources.
    ///
    /// This property shall contain any status or health properties of the resource.
    Status: ?resource.Status = null,
    /// The manufacturer of this power supply.
    ///
    /// This property shall contain the name of the organization responsible for producing the power supply.  This organization may be the entity from whom the power supply is purchased, but this is not necessarily true.
    Manufacturer: ?[]const u8 = null,
    /// The input ranges that the power supply can use.
    ///
    /// This property shall contain a collection of ranges usable by the power supply unit.
    InputRanges: ?[]const InputRange = null,
    /// The state of the indicator LED, which identifies the power supply.
    ///
    /// This property shall contain the indicator light state for the indicator light associated with this power supply.
    IndicatorLED: ?resource.IndicatorLed = null,
    /// The available actions for this resource.
    ///
    /// This property shall contain the available actions for this resource.
    Actions: ?PowerSupplyActions = null,
    /// The location of the power supply.
    ///
    /// This property shall contain the location information of the associated power supply.
    Location: ?resource.Location = null,
    /// The measured input power of this power supply.
    ///
    /// This property shall contain the measured input power, in watt units, of the associated power supply.
    PowerInputWatts: ?core.Decimal = null,
    /// The measured output power of this power supply.
    ///
    /// This property shall contain the measured output power, in watt units, of the associated power supply.
    PowerOutputWatts: ?core.Decimal = null,
    /// The measured efficiency of this power supply as a percentage.
    ///
    /// This property shall contain the measured power efficiency, as a percentage, of the associated power supply.
    EfficiencyPercent: ?core.Decimal = null,
    /// An indication of whether this device can be inserted or removed while the equipment is in operation.
    ///
    /// This property shall indicate whether the device can be inserted or removed while the underlying equipment otherwise remains in its current operational state.  Devices indicated as hot-pluggable shall allow the device to become operable without altering the operational state of the underlying equipment.  Devices that cannot be inserted or removed from equipment in operation, or devices that cannot become operable without affecting the operational state of that equipment, shall be indicated as not hot-pluggable.
    HotPluggable: ?bool = null,
    /// An array of links to resources or objects associated with this power supply.
    ///
    /// This property shall contain an array of links to resources or objects associated with this power supply.
    RelatedItem: ?[]const core.NavProperty(resource.Item) = null,
    /// How many members `RelatedItem` has in total, which is not how many this response carries.
    @"RelatedItem@odata.count": ?i64 = null,
    /// The next page of `RelatedItem`. Present only when this response left members out.
    @"RelatedItem@odata.nextLink": ?core.ODataId = null,
    /// The set of redundancy groups for this power supply.
    ///
    /// This property shall contain an array of links to the redundancy groups to which this power supply belongs.
    Redundancy: ?[]const core.NavProperty(redundancy.Redundancy) = null,
    /// How many members `Redundancy` has in total, which is not how many this response carries.
    @"Redundancy@odata.count": ?i64 = null,
    /// The next page of `Redundancy`. Present only when this response left members out.
    @"Redundancy@odata.nextLink": ?core.ODataId = null,
    /// The link to the assembly resource associated with this power supply.
    ///
    /// This property shall contain a link to a resource of type `Assembly`.
    Assembly: ?core.NavProperty(assembly.Assembly) = null,
};

pub const Voltage = struct {
    /// Where the resource lives.
    @"@odata.id": ?core.ODataId = null,
    /// The version of the resource this value was read at.
    @"@odata.etag": ?core.ODataETag = null,
    /// The OEM extension property.
    ///
    /// This property shall contain the OEM extensions.  All values for properties that this object contains shall conform to the Redfish Specification-described requirements.
    Oem: ?resource.Oem = null,
    /// The unique identifier for the member within an array.
    ///
    /// This property shall contain the unique identifier for this member within an array.  For services supporting Redfish v1.6 or higher, this value shall contain the zero-based array index.
    MemberId: ?[]const u8 = null,
    /// Voltage sensor name.
    ///
    /// This property shall contain the name of the voltage sensor.
    Name: ?[]const u8 = null,
    /// A numerical identifier to represent the voltage sensor.
    ///
    /// This property shall contain a numerical identifier for this voltage sensor that is unique within this resource.
    SensorNumber: ?i64 = null,
    /// The status and health of the resource and its subordinate or dependent resources.
    ///
    /// This property shall contain any status or health properties of the resource.
    Status: ?resource.Status = null,
    /// The reading of the voltage sensor.
    ///
    /// This property shall contain the voltage sensor's reading.
    ReadingVolts: ?core.Decimal = null,
    /// The value at which the reading is above normal range.
    ///
    /// This property shall contain the value at which the `ReadingVolts` property is above the normal range.  The value of the property shall use the same units as the `ReadingVolts` property.
    UpperThresholdNonCritical: ?core.Decimal = null,
    /// The value at which the reading is above normal range but not yet fatal.
    ///
    /// This property shall contain the value at which the `ReadingVolts` property is above the normal range but is not yet fatal.  The value of the property shall use the same units as the `ReadingVolts` property.
    UpperThresholdCritical: ?core.Decimal = null,
    /// The value at which the reading is above normal range and fatal.
    ///
    /// This property shall contain the value at which the `ReadingVolts` property is above the normal range and is fatal.  The value of the property shall use the same units as the `ReadingVolts` property.
    UpperThresholdFatal: ?core.Decimal = null,
    /// The value at which the reading is below normal range.
    ///
    /// This property shall contain the value at which the `ReadingVolts` property is below normal range.  The value of the property shall use the same units as the `ReadingVolts` property.
    LowerThresholdNonCritical: ?core.Decimal = null,
    /// The value at which the reading is below normal range but not yet fatal.
    ///
    /// This property shall contain the value at which the `ReadingVolts` property is below the normal range but is not yet fatal.  The value of the property shall use the same units as the `ReadingVolts` property.
    LowerThresholdCritical: ?core.Decimal = null,
    /// The value at which the reading is below normal range and fatal.
    ///
    /// This property shall contain the value at which the `ReadingVolts` property is below the normal range and is fatal.  The value of the property shall use the same units as the `ReadingVolts` property.
    LowerThresholdFatal: ?core.Decimal = null,
    /// Minimum value for this sensor.
    ///
    /// This property shall indicate the lowest possible value for the `ReadingVolts` property.  The value of the property shall use the same units as the `ReadingVolts` property.
    MinReadingRange: ?core.Decimal = null,
    /// Maximum value for this sensor.
    ///
    /// This property shall indicate the highest possible value for the `ReadingVolts` property.  The value of the property shall use the same units as the `ReadingVolts` property.
    MaxReadingRange: ?core.Decimal = null,
    /// The area or device to which this voltage measurement applies.
    ///
    /// This property shall contain a description of the affected device or region within the chassis to which this voltage measurement applies.
    PhysicalContext: ?physical_context.PhysicalContext = null,
    /// The available actions for this resource.
    ///
    /// This property shall contain the available actions for this resource.
    Actions: ?VoltageActions = null,
    /// An array of links to resources or objects to which this voltage measurement applies.
    ///
    /// This property shall contain an array of links to resources or objects to which this voltage measurement applies.
    RelatedItem: ?[]const core.NavProperty(resource.Item) = null,
    /// How many members `RelatedItem` has in total, which is not how many this response carries.
    @"RelatedItem@odata.count": ?i64 = null,
    /// The next page of `RelatedItem`. Present only when this response left members out.
    @"RelatedItem@odata.nextLink": ?core.ODataId = null,
};

test {
    std.testing.refAllDecls(@This());
}
